<x-niu-layout>
    ...
    <div class="container-fluid">
        <div class="row justify-content-between">
            ...
            <div class="mr-1 justify-content-between row">
                <div class="ml-auto mr-1">
                    {{-- tobedestined --}}
                    <a href="{{ route('rowLabels.create') }}" class="btn btn-info">Tambah Kelompok Baris</a>
                </div>
                {{-- <div class="ml-auto mr-1">
        ...
        @endif
        <table class="table table-hover table-sorted" id="tabel-kelompok-baris">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                @foreach ($rowLabels as $key => $item)
                    <tr>
                        ...
                        <td class="text-center">
                            <form method="POST"
                                action="{{ route('rowLabels.destroy', ['id' => Illuminate\Support\Facades\Crypt::encrypt($item->id)]) }}"
                                class="delete-form">
                                @csrf
                                @method('DELETE')
        ...
        <script>
            ...
            const handleDeleteColumn = function(encryptedId) {
                if (confirm('Are you sure you want to delete this subject?')) {
                    fetch("{{ route('rowLabels.destroy', ['id' => ':id']) }}".replace(':id', encryptedId), {
                            method: 'POST',
                            headers: {
