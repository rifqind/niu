    ...
    <div class="container-fluid">
        <div class="row justify-content-between mb-2">
            ...
            <div class="ml-auto mr-2">
                <a href="{{ route('periode_group.create') }}" class="btn bg-info-fordone"><i
                        class="fa-solid fa-plus"></i> Tambah Kelompok Periode</a>
            </div>
        </div>
        @if (session('success'))
        ...
        <table class="table table-hover table-sorted table-bordered" id="tabel-turtahun">
            ...
            <tbody>
                {{-- {{ dd($columns) }} --}}
                @foreach ($periode_groups as $key => $item)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $item->label }}</td>
                        <td class="text-center">
                            <a href="{{ route('periode_group.edit', ['id' => Illuminate\Support\Facades\Crypt::encrypt($item->id)]) }}"
                                <i class="fa-solid fa-pen" style="color: #1032e0;"></i>
                            </a>
                        </td>
                        <td class="text-center">
                            <form method="POST"
                                action="{{ route('periode_group.destroy', ['id' => Illuminate\Support\Facades\Crypt::encrypt($item->id)]) }}"
                                class="delete-form">
                                @csrf
                                @method('DELETE')
