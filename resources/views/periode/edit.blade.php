<x-niu-layout>
    ...
    <div class="container-fluid">
        ...
        <div class="card">
            <div class="card-body">
                <form id = "periodeForm" method="POST" class="form-horizontal" action="{{ route('periode.update') }}">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control mb-3" name="label" value="{{ $periode->label }}"
                            placeholder="Isi Nama Periode" required>
                        <input class="form-control mb-3 d-none" name="id"
                            value="{{ Illuminate\Support\Facades\Crypt::encrypt($periode->id) }}" readonly>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="type" id="type" class="form-control select2bs4"
                            ...
                            @foreach ($turtahun_groups as $turtahun_group)
                                <option value="{{ $turtahun_group->id }}"
                                    {{ $periode->type == $turtahun_group->id ? 'selected' : '' }}>
