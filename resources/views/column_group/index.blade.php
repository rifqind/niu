        ...
        <table class="table table-hover table-sorted table-bordered" id="tabel-grup-kolom">
            <thead>
                ...
                <tr>
                    ...
                    <td class="text-center search-header tabel-width-70"><input type="text" class="search-input form-control"></td>
            ...
            <tbody>
                {{-- {{ dd($columns) }} --}}
                @foreach ($column_groups as $key => $item)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $item->label }}</td>
                        <td class="text-center">
                            <a href="{{ route('column_group.edit', ['id' => Illuminate\Support\Facades\Crypt::encrypt($item->id)]) }}"
                                <i class="fa-solid fa-pen"></i>
                            </a>
                        </td>
                        <td class="text-center">
                            <form method="POST"
                                action="{{ route('column_group.destroy', ['id' => Illuminate\Support\Facades\Crypt::encrypt($item->id)]) }}"
                                class="delete-form">
                                @csrf
                                @method('DELETE')
